...
class FollowerListVC: UIViewController {
    ...
    func configureDataSource() {
        dataSource = UICollectionViewDiffableDataSource<Section, Follower>(collectionView:
            collectionView, cellProvider: {
                (collectionView, indexPath, follower) -> UICollectionViewCell? in
                let cell = collectionView.dequeueReusableCell(withReuseIdentifier:
                    FollowerCell.reuseID, for: indexPath) as! FollowerCell
...
extension FollowerListVC: FollowerListVCDelegate {
    func didRequestFollower(for username: String) {
        ...
        title = username
        page = 1
        followers.removeAll(keepingCapacity: true)
        filteredFollowers.removeAll(keepingCapacity: true)
